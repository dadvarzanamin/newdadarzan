...
(function ($) {
  ...
  if ($('.cart-table-item').length > 0) {
    function calculateTotals() {
      ...
      document.querySelector(".cart-summary p:nth-child(1) span").textContent = `${subtotal.toFixed(2)}تومان`;

      let tax = subtotal * 0.10;
      document.querySelector(".cart-summary p:nth-child(3) span").textContent = `${tax.toFixed(2)}تومان`;

      let finalTotal = subtotal + tax;
      document.querySelector(".cart-summary p:nth-child(5) span").textContent = `${finalTotal.toFixed(2)}تومان`;
    }
